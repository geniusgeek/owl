...
9. `scroll_left()`: Scroll left the page. It is suitable when you want to see the
elements to the left of the current viewport.
10. `scroll_right()`: Scroll right the page. It is suitable when you want to see the
elements to the right of the current viewport.
11. `back()`: Navigate back to the previous page. This is useful when you want
to go back to the previous page, as current page is not useful.
12. `stop()`: Stop the action process, because the task is completed or failed
(impossible to find the answer). In this situation, you should provide your
answer in your output.
13. `get_url()`: Get the current URL of the current page.
14. `find_text_on_page(search_text: str)`: Find the next given text on the
...
15. `visit_page(url: str)`: Go to the specific url page.
16. `click_blank_area()`: Click a blank area of the page to unfocus the
...
17. `ask_question_about_video(question: str)`: Ask a question about the
current webpage which contains video, e.g. youtube websites.
18. `select_all()`: Select all content on the current page.
19. `copy()`: Copy currently selected content into clipboard.
20. `paste()`: Paste content from clipboard.
...
class BaseBrowser:
    ...
    def init(self) -> None:
        ...
        self.context = self.browser.new_context(accept_downloads=True, viewport={"width": 1920, "height": 1080},screen={"width": 1920, "height": 1080})
        # Create a new page
    ...
    def scroll_right(self) -> None:
        r"""Scroll right the page."""
        
        # Simulate mouse wheel scrolling (100 pixels right)
        self.page.mouse.wheel(100, 0)
        # Ensure scroll with JavaScript for reliability
        self.page.evaluate("window.scrollBy(100, 0)")

    def scroll_left(self) -> None:
        r"""Scroll left the page."""
        # Simulate mouse wheel scrolling (100 pixels left)
        self.page.mouse.wheel(-100, 0)
        # Ensure scroll with JavaScript for reliability
        self.page.evaluate("window.scrollBy(-100, 0)")

    def select_all(self) -> None:
        r"""Select all content on the page."""
        self.page.keyboard.press("Control+A")   

    def copy(self) -> None:
        r"""Copy selected content."""
...
Here is two EXAMPLES of the output (note, this are ONLY EXAMPLES to guide you, not ACTUAL; ):
```json
{{
...
functions like `scroll_up()`, `scroll_down()`, `scroll_left()`, `scroll_right()` to check the full content of
...
- Flexibly use interactive elements like slide down selection bar, or slide right or left to filter
out the information you need. Sometimes they are extremely useful.
- Use `select_all()` which does the same as pressing Ctrl + A for selecting all content on the page, `copy()` which does the same as pressing Ctrl + C for copying selected content to clipboard, `paste()` which does the same as pressing Ctrl + V for pasting content from clipboard.
```
        """
...
"""
        # Here are some tips for you: Please note that we can only see a part of the full page because of the limited viewport after an action. Thus, do not forget to use methods like `scroll_up()`, `scroll_down()`, `scroll_left()`, `scroll_right()` to check the full content of the webpage, because the answer or next key step may be hidden in the content below or to the left or right.
